  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      content:
        "Hey 👋 I’m Ashy. I can help with drag, lift, frontal area, regulations, and report ideas.",
    },
  ]);
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "Ashy could not connect right now. Refresh the page, then try again.",
        },
      ]);
    } finally {
      setLoading(false);
      ...
      <div className="border-t border-slate-200 p-4">
        <div className="flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-full border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#2525bf]"
          />
          ...
          <button
            ...
            onClick={send}
            disabled={loading}
            className="rounded-full bg-[#2525bf] px-5 py-3 text-sm font-semibold text-white disabled:opacity-50"
          >
            Send
          </button>
        </div>
